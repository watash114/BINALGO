...
class Notification
{
    ...
    {
        $stmt = $this->db->prepare(
            "DELETE FROM notifications WHERE created_at < datetime('now', '-' || :days || ' days')"
        );
    ...
    public function getStats(): array
    {
        ...
        $today = $this->db->query("SELECT COUNT(*) FROM notifications WHERE DATE(created_at) = date('now')")->fetchColumn();
        return [
            'total'  => (int)$total,
            'unread' => (int)$unread,
            'today'  => (int)$today,
