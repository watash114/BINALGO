...
class GuidePayout
{
    ...
    {
        $stmt = $this->db->prepare(
            "SELECT DATE_FORMAT(gp.created_at, '%Y-%m') as month,
             ...
             FROM guide_payouts gp
             WHERE gp.guide_id = :gid
               AND gp.created_at >= DATE_SUB(CURDATE(), INTERVAL :months MONTH)
             GROUP BY DATE_FORMAT(gp.created_at, '%Y-%m')
             ORDER BY month ASC"
        );
    ...
    {
        $stmt = $this->db->prepare(
            "UPDATE guide_payouts SET payout_status = 'approved', approved_by = :admin, approved_at = NOW(), updated_at = NOW()
    ...
    {
        $stmt = $this->db->prepare(
            "UPDATE guide_payouts SET payout_status = 'paid', paid_at = NOW(), payout_reference = :ref, updated_at = NOW()
