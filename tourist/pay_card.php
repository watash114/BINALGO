...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['process_card'])) {
    ...
    if ($existing_payment) {
        ...
        $db->prepare("UPDATE payments SET reference_number = :ref, payment_method = 'card', card_last_four = :last4, card_brand = :brand, payment_status = 'pending', updated_at = datetime('now') WHERE id = :id")
