...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['toggle_bookmark'])) {
    ...
    if ($check->fetch()) {
        ...
        $db->prepare("INSERT INTO event_bookmarks (event_id, user_id, created_at) VALUES (:eid, :uid, datetime('now'))")->execute([':eid' => $eid, ':uid' => $user_id]);
    }
    $qs = $_GET;
    unset($qs['page']);
...
if ($pastMode) {
    $where[] = "e.event_start_date IS NOT NULL AND e.event_start_date < date('now')";
} else {
    $where[] = "(e.event_start_date >= date('now') OR e.event_start_date IS NULL)";
}
...
$calMonth = '';
if ($when === 'this_month') {
    $where[] = "strftime('%Y-%m', e.event_start_date) = :month";
...
if ($view === 'calendar') {
    ...
    $calStmt = $db->prepare(
        "SELECT e.*, d.name as destination_name, d.location as destination_location, d.image as dest_image,
         ...
         FROM events e
         LEFT JOIN destinations d ON e.destination_id = d.id
         WHERE e.status = 'published' AND strftime('%Y-%m', e.event_start_date) = :month
...
function addToCalendar(btn) {
    ...
    var lines = ['BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//BINALGO//Events//EN', 'BEGIN:VEVENT',
        'UID:' + Date.datetime('now') + '@binalgo.local',
...
document.querySelectorAll('[data-countdown]').forEach(function (el) {
    ...
    function update() {
        var now = Date.datetime('now'), diff = target - now;
