...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['process_gcash'])) {
    if (!verify_token($_POST['csrf_token'] ?? '')) {
        ...
        redirect("/tourist/pay_gcash.php?booking_id={$booking_id}");
    }
    ...
    if (empty($gcash_number) || empty($gcash_name)) {
        flash_message('error', 'GCash number and account name are required.');
        redirect("/tourist/pay_gcash.php?booking_id={$booking_id}");
    }

    if (!preg_match('/^(09|\+639)\d{9}$/', $gcash_number)) {
        flash_message('error', 'Invalid GCash number format.');
        redirect("/tourist/pay_gcash.php?booking_id={$booking_id}");
    }

    $ref = $paymentModel->generateReferenceNumber();
    $transaction_id = 'gcash_' . bin2hex(random_bytes(12));

    if ($existing_payment) {
        $payment_id = $existing_payment['id'];
        $db->prepare("UPDATE payments SET reference_number = :ref, payment_method = 'gcash', payment_status = 'pending', updated_at = datetime('now') WHERE id = :id")
    ...
    $result = $paymentModel->processGcashPayment($payment_id, $transaction_id, $gcash_number, $gcash_name);

    if ($result['success']) {
        ActivityLog::log($user['id'], 'payment_success', "GCash payment of ₱" . number_format($total, 2) . " for booking #{$booking_id}");
        ...
        $notif_stmt->execute([
            ':uid'   => $user['id'],
            ':title' => 'Payment Confirmed (GCash)',
            ':msg'   => "Your GCash payment of ₱" . number_format($total, 2) . " for {$booking['event_title']} has been confirmed. Reference: {$ref}",
            ':link'  => '/tourist/confirmation.php?payment_id=' . $payment_id,
        ]);

        redirect("/tourist/confirmation.php?payment_id={$payment_id}");
    } else {
        ActivityLog::log($user['id'], 'payment_failed', "GCash payment failed for booking #{$booking_id}: {$result['message']}");
        flash_message('error', $result['message']);
        redirect("/tourist/pay_gcash.php?booking_id={$booking_id}");
    }
}

render_page('tourist', 'pay_gcash.php', 'Pay with GCash', function () use ($booking, $amount, $tax, $service_fee, $total, $booking_id) {
?>
    <div class="row g-4">
        <div class="col-lg-7">
            <div class="card border-0 shadow-sm mb-4">
                <div class="card-header bg-white border-bottom d-flex align-items-center gap-2">
                    <div class="d-flex align-items-center justify-content-center rounded" style="width:32px;height:32px;background:#007dfe;">
                        <span class="text-white fw-bold" style="font-size:0.55rem;">GCash</span>
                    </div>
                    <h5 class="mb-0 fw-semibold">Pay with GCash</h5>
                </div>
                <div class="card-body">
                    <div class="alert alert-light border mb-4">
                        <div class="d-flex align-items-center gap-3 mb-3">
                            <div class="rounded-circle d-flex align-items-center justify-content-center" style="width:48px;height:48px;background:#e8f4fd;">
                                <i class="fas fa-qrcode text-primary" style="font-size:1.3rem;"></i>
                            </div>
                            <div>
                                <div class="fw-semibold">Scan QR Code or Enter Details</div>
                                <small class="text-muted">Use your GCash app to complete payment</small>
                            </div>
                        </div>
                        ...
                        <div class="text-center mb-4 p-4 bg-white rounded border" id="qrSection">
                            <div class="mb-2">
                                <div style="width:180px;height:180px;margin:0 auto;background:#f0f0f0;border:2px dashed #ccc;border-radius:12px;display:flex;flex-direction:column;align-items:center;justify-content:center;">
                                    <i class="fas fa-qrcode text-muted mb-1" style="font-size:4rem;opacity:0.3;"></i>
                                    <small class="text-muted">QR Code</small>
                                </div>
                            </div>
                            <div class="fw-bold text-primary fs-5">₱<?= number_format($total, 2) ?></div>
                            <small class="text-muted">Scan to pay via GCash app</small>
                        </div>
                        ...
                        <form method="POST" id="gcashForm">
                            <?= csrf_field() ?>
                            <input type="hidden" name="process_gcash" value="1">

                            <div class="mb-3">
                                <label class="form-label fw-semibold">GCash Mobile Number <span class="text-danger">*</span></label>
                                <div class="input-group">
                                    <span class="input-group-text bg-white"><i class="fas fa-mobile-alt text-muted"></i></span>
                                    <input type="text" name="gcash_number" class="form-control" placeholder="09XXXXXXXXX" maxlength="11" required
                                           oninput="this.value=this.value.replace(/[^0-9]/g,'')">
                                </div>
                                <small class="text-muted">Enter the mobile number linked to your GCash account</small>
                            </div>

                            <div class="mb-3">
                                <label class="form-label fw-semibold">Account Name <span class="text-danger">*</span></label>
                                <input type="text" name="gcash_name" class="form-control" placeholder="Full name as registered in GCash" required>
                            </div>
                            ...
                            <div class="alert alert-warning small mb-3">
                                ...
                                You will receive a GCash notification on your phone. Approve the payment to complete.
                            </div>

                            <button type="submit" class="btn btn-primary btn-lg w-100" id="payBtn" style="background:#007dfe;border-color:#007dfe;">
                                <i class="fas fa-lock me-2"></i>Pay ₱<?= number_format($total, 2) ?> via GCash
                            </button>
                        </form>
                    </div>
                </div>
...
document.getElementById('gcashForm').addEventListener('submit', function(e) {
    ...
    btn.disabled = true;
    btn.innerHTML = '<i class="fas fa-spinner fa-spin me-2"></i>Processing GCash Payment...';
});
</script>
<?php }); ?>
