...
$events_stmt = $db->prepare(
    "SELECT e.*, s.id as schedule_id, s.start_date, s.start_time, s.end_date, s.end_time, s.available_spots, d.name as dest_name
     ...
     WHERE e.destination_id = :dest_id AND e.status = 'published' AND s.status = 'scheduled' AND s.start_date >= date('now')
    ...
    if (isset($_POST['toggle_bookmark'])) {
        ...
        if ($did > 0) {
            ...
            if ($chk->fetch()) {
                ...
                $db->prepare("INSERT INTO dest_bookmarks (destination_id, user_id, created_at) VALUES (:did, :uid, datetime('now'))")->execute([':did' => $did, ':uid' => $_SESSION['user_id']]);
            }
        }
        redirect('/tourist/destination_detail.php?id=' . $destId);
    }
