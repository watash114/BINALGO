...
$db->exec("UPDATE events SET status = 'completed', updated_at = datetime('now') WHERE status = 'published' AND event_end_date IS NOT NULL AND event_end_date < date('now')");
...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    if ($action === 'publish_event' && $eid) {
        $db->prepare("UPDATE events SET status = 'published', updated_at = datetime('now') WHERE id = :id")->execute([':id' => $eid]);
    ...
    if ($action === 'unpublish_event' && $eid) {
        $db->prepare("UPDATE events SET status = 'draft', updated_at = datetime('now') WHERE id = :id")->execute([':id' => $eid]);
    ...
    if ($action === 'cancel_event' && $eid) {
        $db->prepare("UPDATE events SET status = 'cancelled', updated_at = datetime('now') WHERE id = :id")->execute([':id' => $eid]);
