...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    if ($action === 'confirm' && $bid) {
        $db->prepare("UPDATE bookings SET status = 'confirmed', updated_at = datetime('now') WHERE id = :id AND status = 'pending'")->execute([':id' => $bid]);
    ...
    if ($action === 'cancel' && $bid) {
        $db->prepare("UPDATE bookings SET status = 'cancelled', updated_at = datetime('now') WHERE id = :id AND status IN ('pending','confirmed')")->execute([':id' => $bid]);
    ...
    if ($action === 'complete' && $bid) {
        $db->prepare("UPDATE bookings SET status = 'completed', updated_at = datetime('now') WHERE id = :id AND status = 'confirmed'")->execute([':id' => $bid]);
